...
class Player(pygame.sprite.Sprite):
    ...
    def shoot_laser(self):
        if self.multishot:
            ...
            self.lasers.add(Laser(self.rect.midleft, -8, self.rect.bottom, oblique=True))
            self.lasers.add(Laser(self.rect.midright, -8, self.rect.bottom, oblique=True))
        else:
...
class Laser(pygame.sprite.Sprite):
    def __init__(self, pos, speed, screen_height, oblique = False, direction="left"):
        super().__init__()
    ...
    def update(self):
        ...
        if self.oblique:
            if self.direction == "left":
                self.rect.x -= self.speed  # Move left for left oblique laser
            elif self.direction == "right":
                self.rect.x += self.speed  # Move right for right oblique laser
        self.destroy()
...
class Extra(pygame.sprite.Sprite):
    def __init__(self, side, screen_width):
        super().__init__()
        self.image = pygame.image.load('puan.jpg').convert_alpha()
...
class Game:
    ...
    def extra_shoot(self):
        ...
        if current_time - self.last_shot_time >= self.cooldown:
            if self.extra.sprites():
                item = Item(extra_sprite.rect.center)
                self.items.add(item)
    ...
    def collision_check(self):
        ...
        collided_items = pygame.sprite.spritecollide(self.player.sprite, self.items, True)
